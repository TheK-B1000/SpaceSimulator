...
- EDEN OS Advisory mode posts bounded H contexts to ProjectEden `POST /api/missions/sessions/{id}/advisories`, accepts the four-field §19.3 response, records `EdenAdvisoryIssued` telemetry (issuance time on the event; evaluation/snapshot times in Detail), and surfaces recommendation/rationale on the operator HUD read-only. Observe mode never evaluates or calls `/advisories`.
