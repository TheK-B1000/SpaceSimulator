...
- EDEN OS Advisory mode posts bounded H contexts to ProjectEden `POST /api/missions/sessions/{id}/advisories`, accepts the four-field §19.3 response, records exactly one `EdenAdvisoryIssued` telemetry fact per accepted response (issuance time on the event; evaluation/snapshot times in Detail), and surfaces recommendation/rationale on the operator HUD read-only. Checkpoint I accepted on `main` at `89d47da` + `89761fd`. Observe mode never evaluates or calls `/advisories`.
- EDEN OS Checkpoint J adds an internal validated-command boundary (`UEdenExternalCommandRouter` / `FEdenExternalCommandProposal`). Closed allowlist: `SetThermalControlMode`, `SetLoadShedMode`, `SetPropulsionPriorityMode` using exact 0005 enums. Validation defaults disabled (`bExternalCommandValidationEnabled = false`) and requires `AuthorizedControl` plus the enable flag to return Valid. Validated ≠ executed. Checkpoint J accepted on `main` at `22f8bb9`.
- EDEN OS Checkpoint K adds authorized execution (`UEdenExternalCommandExecutor` / `FEdenValidatedExternalCommand`). Runtime `bExternalCommandExecutionEnabled` defaults false. Execution requires AuthorizedControl + validation enabled + execution enabled; consumes validated artifacts only; converges only through `UEdenOperatorControlComponent`; uses evaluation-scoped cooldown and at-most-one apply attempt per ProposalId; emits `EdenExternalCommandExecuted` on successful convergence with `Source=eden_authorized_control`; must not fabricate human `operator_action`. Checkpoint K accepted on `main` at `a50bcf1`.
- EDEN OS Checkpoint L adds outbound typed command automation: Unreal posts `POST /api/missions/sessions/{session_id}/command-proposals` after an accepted advisory for the same EvaluationId; ProjectEden may return a typed proposal (or 204); Unreal parses exact wire vocabulary into `FEdenExternalCommandProposal`, must pass J, and may call K only when execution is currently enabled. `bExternalCommandAutomationEnabled` defaults false. Production ProjectEden default reasoner proposes nothing; live/test fixture uses `EDEN_COMMAND_PROPOSAL_REASONER=test-load-shed` (deterministic `set_load_shed_mode`/`shed`, not production AI policy). No inbound Unreal command server; no NL→command path; no bespoke `/command-results` (feedback via existing `/events`). Checkpoint L accepted on Unreal `main` at `87a5a97` (+ ready-docs `22102ab`) with ProjectEden `d2822b2`.
- EDEN OS Checkpoint M is verification/closeout of the H→L product loop (no new architecture). Automated live FastAPI mode matrix (Observe / Advisory / AuthorizedControl) and full Unreal `Eden.` / ProjectEden API regression are green as of 2026-08-08; human Editor PIE remains a required acceptance gate and is not self-certified by automation.
