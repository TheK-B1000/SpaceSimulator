...
- EDEN OS Advisory mode posts bounded H contexts to ProjectEden `POST /api/missions/sessions/{id}/advisories`, accepts the four-field §19.3 response, records exactly one `EdenAdvisoryIssued` telemetry fact per accepted response (issuance time on the event; evaluation/snapshot times in Detail), and surfaces recommendation/rationale on the operator HUD read-only. Checkpoint I accepted on `main` at `89d47da` + `89761fd`. Observe mode never evaluates or calls `/advisories`. AuthorizedControl / external command execution remains locked behind Checkpoint J (boundary) then K (modes).
