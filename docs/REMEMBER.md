...
- EDEN OS Checkpoint J adds an internal validated-command boundary (`UEdenExternalCommandRouter` / `FEdenExternalCommandProposal`). Closed allowlist: `SetThermalControlMode`, `SetLoadShedMode`, `SetPropulsionPriorityMode` using exact 0005 enums. Validation defaults disabled (`bExternalCommandValidationEnabled = false`) and requires `AuthorizedControl` plus the enable flag to return Valid. Validated ≠ executed. Checkpoint J accepted on `main` at `22f8bb9`. Authorized execution remains Checkpoint K and must not begin until its contract is locked.
